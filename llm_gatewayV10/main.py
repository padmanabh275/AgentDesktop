...
DEFAULT_ORDER = ["gemini", "nvidia", "groq", "cerebras", "openrouter", "github", "ollama", "openai"]
...
PORT = int(os.getenv("GATEWAY_V10_PORT") or os.getenv("GATEWAY_V9_PORT") or "8110")

# V8: agent_routing.yaml maps `agent="<name>"` to a preferred provider name.
# The caller's explicit `provider=` still wins. Reloaded per request so edits
# apply without restarting the gateway process.
import yaml
_AGENT_ROUTING_PATH = ROOT / "agent_routing.yaml"


def _load_agent_routing() -> dict[str, str]:
    if not _AGENT_ROUTING_PATH.exists():
        return {}
    try:
        data = yaml.safe_load(_AGENT_ROUTING_PATH.read_text()) or {}
        return {str(k): str(v) for k, v in data.items()}
    except Exception as e:  # pragma: no cover - logged then ignored
        print(f"[v10] failed to parse agent_routing.yaml: {e!r}")
        return {}
...
TIER_TO_ORDER = {
    "TINY":  ["github", "openrouter", "groq", "cerebras", "gemini", "nvidia", "ollama", "openai"],
    "LARGE": ["gemini", "groq", "nvidia", "cerebras", "github", "openrouter", "ollama", "openai"],
}
...
app = FastAPI(title="LLM Gateway V10", lifespan=lifespan)
...
async def _resolve_image_urls(messages: list[dict]) -> list[dict]:
    ...
    async def _fetch_to_data_url(url: str) -> str:
        ...
        headers = {
            "User-Agent": "Mozilla/5.0 (compatible; LLMGatewayV10/0.1; +image-resolver)",
            "Accept": "image/*,*/*;q=0.8",
        }
...
async def chat(req: ChatRequest):
    ...
    if req.agent and not req.provider:
        pinned = _load_agent_routing().get(req.agent)
...
async def index():
    return FileResponse(
        str(ROOT / "static" / "dashboard.html"),
