"""Python client for LLM Gateway V10. Adds agent/session tagging, a
batch endpoint, streaming, vision (via /v1/chat messages or /v1/vision),
and exposes the gateway's `retries` count in the response.

V10 behaviour summary (caller-visible):
...
DEFAULT_URL = (
    os.getenv("LLM_GATEWAY_V10_URL")
    or os.getenv("LLM_GATEWAY_V9_URL")
    or "http://localhost:8110"
)
...
class LLM:
    ...
    def vision(self, image: str, prompt: str, **kw) -> dict:
        """POST /v1/vision — single image + prompt. Pass data: or http(s) URL."""
        body = {"image": image, "prompt": prompt, **kw}
        body = {k: v for k, v in body.items() if v is not None}
