...
_env_sessions = os.getenv("AGENT_SESSIONS_ROOT", "").strip()
if _env_sessions:
    SESSIONS_ROOT = Path(_env_sessions)
...
def _atomic_write(path: Path, data: bytes | str) -> None:
    """Write via temp file + replace. Retries on Windows/OneDrive lock races."""
    ...
    try:
        ...
        for attempt in range(8):
            try:
                ...
                if getattr(e, "winerror", None) == 5:
                    ...
                    continue
                raise
        # OneDrive / AV / concurrent readers can block rename on Windows.
        if isinstance(data, bytes):
        ...
        if last_err is not None:
            import sys
            print(
                f"[persistence] WARNING: atomic replace failed for {path} "
