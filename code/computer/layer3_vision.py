"""Layer 3: vision capture_mode + pixel clicks via cua-driver + gateway vision."""
    ...
    *,
    ...
    if is_canvas_fixture_goal(goal):
        if _FIXTURE_CANVAS.is_file():
            ...
            launch_pid = int(launched.get("pid") or 0) if isinstance(launched, dict) else 0
        ...
        if not pid or not window_id:
            ...
            if isinstance(launched, dict) and launched.get("pid"):
    ...
    for turn in range(1, max_steps + 1):
        ...
        if reply is None:
            return VisionResult(
                success=False, note="vision gateway unavailable",
                turns=turn, pid=pid, window_id=window_id,
            )
...
def _fallback_launch_browser_window(listed: dict) -> tuple[int | None, int | None]:
...
def _resize_for_vision(
    ...
    *,
    ...
    """Downscale screenshot for gateway vision; return scale factors for clicks."""
    raw = image_url.split(",", 1)[-1]
