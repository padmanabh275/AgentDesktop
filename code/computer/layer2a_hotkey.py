...
async def run_hotkey_script(
    ...
    *,
    ...
    for step in script:
        ...
        if tool == "launch_app":
            ...
            if isinstance(out, dict):
                pid = int(out.get("pid") or pid or 0) or pid
                wins = out.get("windows") or []
                if wins:
            ...
            if "calc" in app.lower() and pid and window_id:
                out = _type_calculator_expr(client, pid, window_id, str(args.get("text") or ""))
            else:
                out = client.type_text(pid=int(args.get("pid") or pid or 0),
    ...
    if pid and window_id and "calc" in app.lower():
        ...
        actions.append({"tool": "get_window_state", "args": {"pid": pid, "window_id": window_id}, "result": snap})
    ...
    *,
    ...
    for _ in range(attempts):
        ...
        if not wins:
            listed = client.list_windows()
            wins = [
                w for w in (listed.get("windows") or [])
