...
typed BrowserOutput / AgentResult contract, and owns the layer cascade:
    ...
    Layer 2b — A11yDriver        (text-only, V10 /v1/chat)
    Layer 3  — SetOfMarksDriver  (vision, V10 /v1/vision)
...
_UA = (
    "Mozilla/5.0 (compatible; S10-Browser-Skill/0.1; +llm_gatewayV10)"
)
...
class BrowserSkill:
    ...
    def __init__(self, *, gateway_url: str = "http://localhost:8110",
        ...
        # Forwarded to V10 so the gateway ledger can attribute each call to
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        t0 = time.time()
        client = V10Client(base_url=self.gateway_url, agent=self.agent_tag,
        ...
        if a11y_result.success:
            return self._pack_driver("a11y", url, goal, a11y_result,
                                     ...
                                     elapsed=time.time() - t0)
        if force_path == "a11y":
            # Caller pinned a11y — do not escalate to vision.
            if getattr(a11y_result, "actions", None) or getattr(a11y_result, "extracted", None):
                return self._pack_driver("a11y", url, goal, a11y_result,
                                         final_url=getattr(a11y_result, "final_url", url),
                                         elapsed=time.time() - t0)
            return self._pack_error(
                url, goal, "interaction_failed",
                a11y_result.note or "a11y layer exhausted (force_path=a11y)",
                elapsed=time.time() - t0,
            )
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    out = DriverResult(
                        success=False,
                        note=f"{DriverCls.LAYER_NAME} error: {type(e).__name__}: {e}",
                    )
                    out.final_url = page.url
                    out.extracted = ""
                    try:
                        out.extracted = _extract(await page.content())
                    except Exception:                          # noqa: BLE001
                        pass
                    out.turns = len(drv.steps)
                    out.actions = [
