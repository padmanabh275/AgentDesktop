...
from cua.tools import cua_tool_payload
...
def tool_payload(tool_names: list[str]) -> list[dict] | None:
    ...
    out: list[dict] = []
    for n in tool_names:
        if n in _TOOL_CATALOG:
            out.append(_TOOL_CATALOG[n])
    out.extend(cua_tool_payload(tool_names))
    return out or None
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    skills are LLM-backed and route through the V10 gateway with
    ...
    if skill.name == "computer":
        ...
        from computer.goal_utils import enrich_computer_metadata
        meta = enrich_computer_metadata(meta, query)
        node_spec = NodeSpec(
            skill="computer",
            inputs=node_dict.get("inputs") or [],
            metadata=meta,
    ...
    # Retriever returning found:false is an upstream miss — queue recovery
    # so the Planner can switch to researcher/browser instead of dead-ending
    # at a formatter with empty INPUTS.
    if skill.name == "retriever" and parsed.get("found") is False:
        summary = str(parsed.get("summary") or "no relevant chunks in knowledge base")
        return AgentResult(
            success=False,
            ...
            elapsed_s=time.time() - started,
            provider=reply.get("provider", ""),
            error=f"retriever miss: {summary}",
        ), rendered
