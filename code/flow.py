...
from schemas import AgentResult, FlowRunResult, NodeState
...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
                  resume: bool = False, quiet: bool = False) -> FlowRunResult:
        ...
        if memory_hits:
            _log(f"[memory.read] {len(memory_hits)} hit(s) visible to every skill this run")
        try:
            memory_svc.remember(query, source="user_query", run_id=sid)
        except Exception as e:
            _log(f"[memory.remember] skipped: {e!r}")
        ...
        while True:
            ...
            if executed_count + len(ready) > MAX_NODES:
                _log(f"[flow] node cap {MAX_NODES} hit at {executed_count}; stopping")
                break
            ...
            for nid, result, prompt in outcomes:
                ...
                _log(f"[{nid}] {graph.g.nodes[nid]['skill']:18s} "
                ...
                if result.success:
                    ...
                    if decision.action == "skip":
                        _log(f"  ↪ {nid} failed ({decision.reason}, "
                    ...
                    _log(f"  ↪ recovery ({decision.reason}): planner node "
        ...
        return FlowRunResult(session_id=sid, answer=formatter_answer or "")
...
def main() -> None:
    ...
    result = asyncio.run(Executor().run(query, session_id=resume_sid, resume=bool(resume_sid)))
    if result.answer:
        print(result.answer)
