...
ErrorCode = Literal[
    ...
    "app_not_found",         # desktop target app / cua-driver unavailable
]
...
class ComputerOutput(BaseModel):
    ...
    app: str
    goal: str
    path: Literal["read", "hotkey", "electron", "ax", "vision"]
    turns: int = 0
...
class FlowRunResult(BaseModel):
    """Structured result from flow.Executor.run (CLI + Agent Desktop UI)."""
